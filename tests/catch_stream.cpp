...
//std::cerr << "-- adding char [" << static_cast<int>(c) << "]\n";
...
//std::cerr << "-- read char [" << static_cast<int>(c) << "]\n";
    ...
    CATCH_START_SECTION("input_stream: load from input file stream")
    {
        ...
        //
        std::stringstream ss;
        ss << SNAP_CATCH2_NAMESPACE::g_tmp_dir() << "/strandard_input.txt";
        std::string const filename(ss.str());
        ...
        //
        CATCH_REQUIRE(freopen(filename.c_str(), "r", stdin) != nullptr);

        as2js::cin_stream::pointer_t in(std::make_shared<as2js::cin_stream>());

        // The filename for the standard input is set to "-" by default
        //
        CATCH_REQUIRE(in->get_position().get_filename() == "-");
        CATCH_REQUIRE(in->get_position().get_line() == 1);

        libutf8::utf8_iterator it(str);
        for(;; ++it)
        {
            std::int32_t const c(in->read_char());
//std::cerr << "-- read char [" << static_cast<int>(c) << "]\n";
            if(c == EOF)
            {
                CATCH_REQUIRE(it == str.end());
