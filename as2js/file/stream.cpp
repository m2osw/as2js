...
std::int32_t base_stream::read_char()
{
    for(;;)
    {
        std::size_t idx(0);
        byte_t buf[8] = {};
        if(f_last_byte != NO_LAST_CHAR)
        {
            buf[0] = f_last_byte;
            f_last_byte = NO_LAST_CHAR;
        }
        else
        {
            int const c(get_byte());
            if(c == EOF)
            {
                return EOF;
            }
            buf[0] = static_cast<byte_t>(c);
        }
        ++idx;
        if(buf[0] < 0x80)
        {
            // regular ASCII character no need to spend more time on anything here
            //
            return buf[0];
        }

        // throw away invalid bytes
        //
        // in a valid input stream, this should never happen
        //
        while(buf[0] >= 0x80 && buf[0] <= 0xBF)
        {
            buf[0] = get_byte();
        }
        if(buf[0] < 0x80)
        {
            // after we threw away invalid bytes, we found an ASCII character
            //
            return buf[0];
        }
        ...
        //
        // TODO: compute size and make sure we read the exact size
        //
        for(; idx < sizeof(buf); ++idx)
        {
            int const c(get_byte());
            if(c == EOF)
            {
